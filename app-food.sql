CREATE DATABASE db_node33_homework

use db_node33_homework

#Tạo bảng không có khóa phụ 
CREATE TABLE user (
...
('Hủ tiếu khô', 'hutieukho.jpg', 35.000, 'Hủ tiếu khô siêu ngon', 1),
('Phở', 'pho.jpg', 40.000, 'Phở siêu ngon', 2),
('Bún riêu', 'bunrieu.jpg', 35.000, 'Bún riêu siêu ngon', 2),
('Lẩu gà lá giang', 'lauga.jpg', 150.000, 'Lẩu gà siêu ngon', 3),
('Lẩu hải sản', 'lauhaisan.jpg', 200.000, 'Lẩu hải sản siêu ngon', 3),
('Nem nướng', 'nemnuong.jpg', 30.000, 'Nem nướng siêu ngon', 1)
...
FROM restaurant
INNER JOIN (
	SELECT res_id, COUNT(user_id) AS total_like
	FROM like_res
	GROUP BY res_id
	ORDER BY total_like DESC
	LIMIT 2
